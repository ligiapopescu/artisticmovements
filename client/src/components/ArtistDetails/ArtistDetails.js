...
export default function ArtistDetails(props) {
  ...
  return (
    <Container className="artist-details" id={props.id}>
      <Row>
        <Col className="artist-details__name">{props.name}</Col>
        <Col>
          {props.hasOwnerRights && <Button onClick={() => removeArtist(props.id)} variant="danger" className="artist-details__remove-artist">
            Remove Artist
          </Button>}
        </Col>
      </Row>
      {props.hasOwnerRights && <AddNewArtwork artistId={props.id}/>}
      <ArtworksList artworks={props.artworks} hasOwnerRights={props.hasOwnerRights}/>
    </Container>
  );
}
